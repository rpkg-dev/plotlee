...
-   The Python module availability checks should ideally be performed in [plotly::kaleido()](https://github.com/plotly/plotly.R/blob/master/R/kaleido.R) via
    [`reticulate::py_require(packages = c("plotly", "kaleido))`](https://rstudio.github.io/reticulate/dev/articles/package.html) during `.onLoad()`.

    Submit a PR upstream doing this (plus our `reticulate::py_module_available()`-based check instead of the dummy `reticulate::import("plotly")` call) and
    remove the `has_python` check here as well as our own `reticulate::py_require()` call during `.onLoad()` (which would become redundant). Note that
    `reticulate::py_require()` requires reticulate \>= 1.41.
...
-   To initialize reticulate's default shared Python virtualenv `r-reticulate`, run `reticulate::virtualenv_create()`. Before reticulate
    [v1.31](https://github.com/rstudio/reticulate/releases/tag/v1.31), it nudged to install miniconda if this virtualenv didn't exist. Version 1.31 of
    reticulate started to automatically propose creating this virtualenv 🎉!

    reticulate [v1.41](https://github.com/rstudio/reticulate/releases/tag/v1.41.0) introduced a [new Python initialization
    workflow](https://rstudio.github.io/reticulate/articles/package.html) based on
    [`reticulate::py_require()`](https://rstudio.github.io/reticulate/reference/py_require.html) which automatically creates ephemeral Python venvs if needed
...
#'   `r c(formats_raster, formats_postscript) |> pal::as_md_vals() |> pal::enum_str()`.
